...
$(document).ready(function () {
  $('.list-missions').each(function () {
    ...
    for (const mission of MissionsModule.getAllMissions()) {
      const content = MissionsModule.getMission(mission);

      if (!['quidditch', 'test', 'clase'].includes(content.type)) {
        if (content.closed) {
          list.closed.push(mission);
        } else {
          if (content.link) {
            list.progress.push(mission);
          } else {

            let available = true;

            $.each(content.dependency, function (key, depends) {
              if (!MissionsModule.getMission(depends).closed) { available = false; }
            });

            if (available) { list.availables.push(mission); }
            else { list.blocked.push(mission); }
          }
        }
      }
    }

    $.each(list, function (status, misiones) {
      const blocked = status.includes('blocked');
      const availables = status.includes('availables');
      const $box = $('<div>').addClass('listed-missions').addClass('listed-' + status);
      const $cont = $('<cont>');

      for (const key of misiones) {
        const m = MissionsModule.getMission(key);
        if (!m) return null;

        if (blocked) {
          $box.append('<span class="listed-item">' + m.type + ': ' + key + '</span>');
        } else if ((!m.tagged.includes("quidditch")) && (!m.tagged.includes("clase"))) {
          let requis = '';
          $.each(m.requisitos, function (key, r) {
            requis = requis.concat('<req>' + r + '</req>');
          });

          if (requis === '') { requis = '<req>Sin requisitos</req>'; }

          let mus = ''; let us = [];
          $.each(m.users, function (key, u) {
            const user = u.replaceAll('c', '').replaceAll('i', '');
            let creator = ''; let interrogated = '';
            if (u.includes('c')) { creator = ' (Creador)'; }
            if (u.includes('i')) { interrogated = ' (Interrogado - Adicional)'; }
            if ((CharactersModule.getById(user)) && !admins.includes(user)) { us.push(CharactersModule.getById(user).nombreUsuario + creator + interrogated); }
          });

          $.each(us.sort(), function (key, u) {
            mus = mus.concat('<insc>' + u + '</insc>');
          });

          let dependencia = '';
          $.each(m.dependency, function (key, d) {
            dependencia = dependencia.concat('<depen>Para poder iniciar la misión debe participar un personaje que este al tanto de la misión y/o investigación obtenidos en "' + d + '".</depen>');
          });

          let original = ''; let originalClass = '';
          if (m.tagged.includes("original")) {
            original = original.concat('<depen>Esta misión fue generada por usuarios, por lo tanto, su participación no se encuentra incluida de los límites de misiones impuestos.</depen>');
            originalClass = ' original-mission';
          }

          let topic = '';

          if (m.link) {
            topic = '<br /><br /><strong>Link:</strong> <a href="' + m.link + '">' + m.type + '</a>';
          }

          $cont.append('<mission><div class="mission-header' + originalClass + '" style="--image: url(' + m.image + ');"><label>' + m.type + '<i class="fa-thin fa-dash"></i>' + key + '</label></div><br><div class="mission-content"><span class="mission-req">' + requis + '</span><br><br><span class="mission-info">' + m.description + dependencia + original + '<br /><br /><strong>Participantes:</strong>' + mus + topic + '</div><br></mission>');
        }
      }

      const s = { 'progress': 'Abiertas', 'closed': 'Cerradas', 'availables': 'Disponibles', 'blocked': 'Bloqueadas' };

      if (blocked) { $this.append('<span class="guidebook-hide show-results"><line>Misiones ' + s[status] + ': ' + misiones.length + '</line><cont class="list-shorted">' + $box.html() + '</cont></span>'); }
      else {
        if (availables) { $this.append('<span class="guidebook-hide show-results visible-results"><line>Misiones ' + s[status] + ': ' + misiones.length + '</line><cont>' + $cont.html() + '</cont></span>'); }
        else {
          $this.append('<span class="guidebook-hide show-results"><line>Misiones ' + s[status] + ': ' + misiones.length + '</line><cont>' + $cont.html() + '</cont></span>');
        }
      }
    });
  });

  $('.bring-mission').each(function () {
    const userLogged = '/u' + _userdata.user_id;
    const target = ($(this).attr('target') || '').toLowerCase();
    const $this = $(this);

    if (MissionsModule.getMission(target)) {
      const mis = MissionsModule.getMission(target);

      $.each(mis.actions, function (key, value) {
        changeResult[key] = value;
      });

      const creat = mis?.creatures || [];

      $.each(creat, function (npc, value) {
        if (!_creatures[npc]) { _creatures[npc] = {}; }
        if (value.ataques) { _creatures[npc]["ataques"] = value.ataques; }
        if (value.defensas) { _creatures[npc]["defensas"] = value.defensas; }
        if (value.atributos) { _creatures[npc]["atributos"] = value.atributos; }
        if (value["clasificación"]) { _creatures[npc]["clasificación"] = value["clasificación"]; }
        if (value["descripción"]) { _creatures[npc]["descripción"] = value["descripción"]; }
        if (value.imagen) { _creatures[npc]["imagen"] = value.imagen; }
        if (value.peligrosidad) { _creatures[npc]["peligrosidad"] = value.peligrosidad; }
        if (value.pv) { _creatures[npc]["pv"] = value.pv; }
        if (value.rareza) { _creatures[npc]["rareza"] = value.rareza; }
        if (value.repele) { _creatures[npc]["repele"] = value.repele; }
      });

      createSearchs(mis.searchs, $this, target, userLogged, mis, true);
      createCards(mis.cards, $this, target, userLogged, mis, true);
  ...
  $(".show-data").each(function () {
    ...
    const target = ($(this).attr('target') || '').toLowerCase();
    const dato = ($(this).attr('dato') || '').toLowerCase();

    for (const user of CharactersModule.getAllIds()) {
      const record = CharactersModule.getById(user);

      switch (type) {
        case "atributos":
          if (records[type][target] >= parseInt(dato)) { available.push(user); }
          break;
        ...
        case "rango":
          if (haveRango(records, dato)) { available.push(user); }
          break;
      }
    }

    if (available.includes(userLogged)) {
      $(this).addClass('show-visible');
    } else {
      $(this).remove();
    }
  ...
  $(".bring-mission-rules").each(function () {
    const $box = $(this);
    const target = $box.attr('target') || 'mission';

    let rules = [
      'Todo lo descubierto en la trama, solo podrá ser sabido por los personajes previamente mencionados, cualquier comentario realizado en multicuentas o personajes que no hayan participado de la trama, será considerado <strong>metarol</strong>, a excepción de información que sea expuesta en "El Profeta" o sea comunicada por uno de los presentes.',
      'Todas las misiones cuentan con <i><strong>Tarjetas</strong></i> que cuentan información adicional, esta solo podrá ser vista por personajes que cumplan algún requisito como: atributos, conocimientos, empleo o sea un usuario con información adicional por relacionamiento.',
      'Los personajes solo podrán mencionar <u>una tarjeta</u> por rol, es decir, no se pueden hacer menciones de más de una tarjeta <strong>on y off rol</strong>. Esta medida la administración la considera la más importante, debido a que la idea es que todos los personajes puedan <strong>participar</strong>. Irrumpir este acuerdo, implicará la expulsión inmediata de la misión.',
      '<strong>Investigar lugares</strong>, implicará colocar el código indicado en el post y lanzar un dado de "Acción". Los resultados no cambiarán según el resultado, pero se revelarán nuevas tarjetas que cumplirán las mismas condiciones anteriores.',
      'Se recuerda que el uso de <strong>determinados hechizos</strong>, pueden revelar más información, abriendo más caminos a la misión.',
      'Todos los personajes contarán con un lapso de <strong>una semana</strong> para intervenir en la misión. Hasta que no se hayan pasado todos los personajes, los usuarios no podrán volver a intervenir. Sin embargo, pasada la semana, puede volver a responder siempre que <strong>otro personaje</strong> haya pasado con anterioridad para evitar el doble post.',
      'Ante cualquier consulta, la administración se encontrará disponible por los canales correspondientes.',
      'Solo podrán reclamar conocimientos en base a los <u>hechizos utilizados</u> o <u>conocimientos aplicados</u>.'
    ];

    if (target != "mission") {
      rules = [
        'Los personajes podrán ir a la sala que deseen, pero deberán hacerlo en grupo. Es decir, si un integrante investiga una de las actividades, el resto deberá seguirlo y completar la misma antes de ir a la siguiente.',
        'Podrán acceder a todas las salas y obtener los puntos de conocimientos correspondientes a las mismas.',
        '<strong>Acciones de Investigar</strong>, implicará colocar el código indicado en el post y lanzar un dado de "Acción". Los resultados no cambiarán según el resultado, pero se revelarán nuevas tarjetas que cumplirán las mismas condiciones anteriores.',
        'Se recuerda que el uso de <strong>determinados hechizos</strong>, pueden revelar más información, abriendo más caminos a la misión.',
        'Todos los personajes contarán con un lapso de <strong>una semana</strong> para intervenir en el tema. Hasta que no se hayan pasado todos los personajes, los usuarios no podrán volver a intervenir. Sin embargo, pasada la semana, puede volver a responder siempre que <strong>otro personaje</strong> haya pasado con anterioridad para evitar el doble post.',
        'Ante cualquier consulta, la administración se encontrará disponible por los canales correspondientes.',
        'Solo podrán reclamar conocimientos en base a los <u>hechizos utilizados</u> o <u>conocimientos aplicados</u>.'
      ];
    }
    ...
    let party = [];
    const mission = ($('.bring-mission').attr('target') || '').toLowerCase();
    const users = (MissionsModule.getMission(mission)?.users || []);

    $.each(users, function (key, user) {
      const u = user.replaceAll('c', '').replaceAll('i', '');
      const character = CharactersModule.getById(u);
      if ((character) && (character.status)) {
        party.push(character.nombreUsuario);
      }
    });

    $box.append('<span class="mission-rule">Solo los siguientes <strong>personajes</strong> podrán participar de la misión: ' + party.sort().join(', ') + '</span>');
    $box.append('<span class="mission-rule">Los usuarios deberán utilizar la <a href="/t23-tablas-codigos#5982">tablilla de combate</a> durante la duración del tema.</span>');

    $.each(rules, function (key, rule) {
      $box.append('<span class="mission-rule">' + rule + '</span>');
    });
  });

  function haveRango(records, dato) {
    let r = false;
    let e = false;

    if (records.status) {
      r = records.rangos.some(rango => rango.toLowerCase().includes(dato));
      $.each(records.experiencia, function (key, exp) {
        if (exp.rango.toLowerCase().includes(dato)) { e = true; }
      });
    }
...
function validateInformation(visibility, validation, card) {
  const userLogged = '/u' + _userdata.user_id;
  ...
  for (const user of CharactersModule.getAllIds()) {
    ...
    if (record.status) {
      // valida nombre
      if (record.nombreUsuario.toLowerCase().includes(card.visible.name)) {
        visibility.push(user);
        validation.push('nombre: ' + card.visible.name);
      }

      // valida empleo
      $.each(record.rangos, function (key, job) {
        if (job.toLowerCase().includes(card.visible.job)) {
          visibility.push(user);
          validation.push('empleo: ' + card.visible.job);
        }
      });

      $.each(record.experiencia, function (key, job) {
        if (job.rango.toLowerCase().includes(card.visible.job)) {
          visibility.push(user);
          validation.push('empleo: ' + card.visible.job);
        }
      });
      ...
      // valida atributos
      $.each(card.visible.attr, function (key, min) {
        if ((min != "min") && (record.atributos) && (parseInt(record.atributos[key]) >= parseInt(min))) {
          visibility.push(user);
          validation.push('atributo: ' + key + ' mayor a ' + min);
        }
      });

      // valida conocimientos
      $.each(card.visible.know, function (key, min) {
        if ((min != "min") && (record.conocimientos) && (parseInt(record.conocimientos[key]) >= parseInt(min))) {
          visibility.push(user);
          validation.push('conocimiento: ' + key + ' mayor a ' + min);
        }
      });
    }
  }
...
function createSearchs(searchs, $this, target, userLogged, mis, root = false) {
  $.each(searchs, function (key, value) {
    _actions[key] = {};

    if (target === "quidditch") {
      const k = Object.keys(value.visible.know)[0];
      _actions[key]['conocimientos'] = k;
      _actions[key]['experiencia'] = value.visible.know[k];
    } else {
      _actions[key]['conocimientos'] = "solo misiones";
    }

    const content = $('<div>');
    $(content).append($(value.cont).attr('search', key).addClass('check-cards'));
    value.cont = $(content).html();

    _actions[key]['result'] = {};
    _actions[key]['requisitos'] = [];
    _actions[key]['result']['leve'] = value.cont;
    _actions[key]['result']['moderado'] = value.cont;
    _actions[key]['result']['critico'] = value.cont;
    _actions[key]['result']['exitoso'] = value.cont;

    let visibility = [];
    let validation = [];

    validateInformation(visibility, validation, value);

    visibility = [...new Set(visibility)];
    validation = [...new Set(validation)];

    let users = validateMissionUsers(mis.users);
    const v = (((visibility.includes(userLogged)) && (users.includes(userLogged))) || (admins.includes(userLogged)));
    const c = '<div class="mission-search"><topes><i class="fa-solid fa-magnifying-glass"></i></topes><req>Solo visible para ' + validation.join(', ') + '</req><span>Para obtener los "' + key + '", tirar un dado numérico copiando el siguiente código en el post de la lanzada:<br /><br /><dl class="codebox"><dd><code>&lt;span class="do-action" target="actions" data="' + key + '"&gt;&lt;/span&gt;</code></dd></dl></span></div>';

    showInformation($this, target, v, root, value.place, c, "search", key, target === "quidditch");
  });
};

function validateMissionUsers(users) {
  const result = new Set(users);

  users.forEach((item) => {
    if (item.includes("c/u") || item.includes("i/u")) {
      let u = item.replaceAll("i", "").replaceAll("c", "");
      result.add(u);
    }
  });
...
function createCards(cards, $this, target, userLogged, mis, root = false) {
  $.each(cards, function (key, card) {
    let visibility = [];
    let validation = [];

    validateInformation(visibility, validation, card);

    visibility = [...new Set(visibility)];
    validation = [...new Set(validation)];

    const content = $('<div>');
    $(content).append($(card.cont).attr('card', key).addClass('check-cards'));
    card.cont = $(content).html();

    let users = validateMissionUsers(mis.users);

    const v = (((visibility.includes(userLogged)) && (users.includes(userLogged))) || (admins.includes(userLogged)));
    const c = '<div class="mission-card"><topes><i class="fa-solid fa-eye-low-vision"></i></topes><req>Solo visible para ' + validation.join(', ') + '</req>' + card.cont + '</div>';

    showInformation($this, target, v, root, card.place, c, "search", key);
  });
};

function showInformation($this, target, visibility, root, place, cont, query, clave, quidditch = false) {
  ...
  if (target === "quidditch") {
    ...
    if ((root) && (!place)) {
      ...
      const selector = '[' + query + '="' + place + '"]';
      $this.find(selector).append(cont);
      if (root) { $(selector).append(cont); }
    }
  }
};

function bringAditionals($this) {
  $('.bring-mission').each(function () {
    const userLogged = '/u' + _userdata.user_id;
    const target = ($(this).attr('target') || '').toLowerCase();
    ...
    if (MissionsModule.getMission(target)) {
      ...
      createSearchs(mis.searchs, $this, target, userLogged, mis);
      createCards(mis.cards, $this, target, userLogged, mis);
    }
  });
